...
the size of the MCS), maintaining arc consistency may filter values, and tighter constraints
...
filtering (Both). We show only results using the best variant for each class, i.e.\ MAC for labelled
graphs, and FC for unlabelled graphs, as the other results are very similar.  On labelled graphs, we
observe that branching often outperforms filtering, and that combining both is rarely much worse
than just doing one or the other, and is often much better, even if on average it is slightly
slower.  On unlabelled graphs, we observe that the three variants have rather similar performance.
