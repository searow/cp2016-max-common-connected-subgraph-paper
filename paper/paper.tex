...
\nl \Begin{
    ...
    \nl $G \gets \FuncSty{associationGraph}(G_1, G_2)$ \;
...
\nl \Begin{
    ...
    \nl \While{$\FuncSty{length}(\mathit{colourClasses}) > 0$}{
        \nl \ForEach{$v \in \FuncSty{last}(\mathit{colourClasses}) \textnormal{~in reverse order}$}{
            ...
            \mathit{solution}'$}
            \nl $\mathit{branchable}' \gets \mathit{branchable} \cup \FuncSty{connected}(G, v)$ \;
...
\nl \Begin{
    ...
    \nl $E \gets \{ ((v_1, v_2), (w_1, w_2)) \in V \times V : \FuncSty{compatible}((v_1, v_2), (w_1,
    w_2)) \}$ \;
    \nl $\KwSty{return} ~ (V, E)$ \;
}
