...
their extension to directed or labelled graphs is discussed in \cref{extension}.
...
\operatorname{V}(G) \times \operatorname{V}(G)$---to avoid confusing vertices of $G \modularproduct
H$ with vertices of the two original graphs, we call vertices of $G \modularproduct H$
\emph{matching nodes}, as each vertex $(u, v)$ of $G \modularproduct H$ denotes the matching of $u$
with $v$. The edges of $G \modularproduct H$ connect matching nodes which denote compatible
...
may not be mapped to vertices with loops.
...
sparser for clique-based approaches.

?? More on AC here? If we think carefully about the size of the domains of the objective variable:
tightening the constraints doesn't really help unless you've made an assignment, right?
...
\cite{DBLP:journals/cor/SegundoRJ11,DBLP:journals/ol/SegundoMRH13}. We compare this to the CP
implementation of Ndiaye and Solnon \cite{DBLP:conf/cp/NdiayeS11}.  We perform our experiments on
machines with Intel Xeon E5-2640 v2 CPUs and 64GBytes RAM; software was compiled using GCC 4.9, and
a timeout of one hour was used.
...
\begin{figure}[p]
    ...
    \caption{The cumulative number of instances solved in under a certain time: on the top, 33\%
        labelled graphs, and in the middle, unlabelled and undirected graphs. On the bottom row, an
...
The top two plots of \cref{figure:unconnected-cumulative} display the cumulative number of instances
...
specific branching rule which selects the next variable to be assigned. Let $\mathit{CComp}$ be the
...
A more conventional CP approach would be to express connectedness as a conventional constraint. For
...
previously found. Therefore, we introduce a new filtering algorithm to ensure connectedness) (resp.
...
\begin{figure}[tb]
    ...
    \caption{A pairwise comparison of different CP techniques. ?? This looks horrible! Make it look
        better.} \label{figure:connected-cp}
\end{figure}
...
specific branching (Branching), by specific filtering (Filtering), or by combining specific
branching and filtering (Both). We show only results using MAC for labelled graphs, and FC for
unlabelled graphs, as the other results are very similar.  On labelled graphs, we observe that
branching often outperforms filtering, and that combining both is rarely much worse than just doing
one or the other, and is often much better, even if on average it is slightly slower.  On unlabelled
graphs, we observe that the three variants have rather similar performance.
...
between the two stages, although it may use more colours than a single colouring would\footnote{What
    if we did not guarantee that vertices in $\mathit{remaining} \cap \mathit{connected}$ came last,
    and just used a conventional colouring with the branching rule?  Suppose we had four vertices in
    $\mathit{remaining}$, and produced a colouring $[[v_1, v_2], [v_3], [v_4]]$, and suppose that
    extending $\mathit{solution}$ with $\{ v_1, v_3, v_4 \}$ gives an optimal solution. If $v_4$ was
    not $\mathit{connected}$ yet, we would not branch on that subtree, and the bound could eliminate
    branching on $v_3$ and $v_1$, so we would miss the solution. Thus we cannot simply add the
branching rule without also adapting the combined bound / ordering heuristic.}.
...
\cref{figure:connected-cumulative}
...
SIP invariants when very similar graphs (except these are easy).
...
Parallel. ?? Stuff on EHPs, how work stealing matters here too \cite{DBLP:journals/jco/BatsynGMP14,DBLP:journals/jcc/KoncDTRJ12,DBLP:journals/algorithms/McCreeshP13,DBLP:journals/topc/McCreeshP15,DBLP:journals/cor/SegundoLP16}
