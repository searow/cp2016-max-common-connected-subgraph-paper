...
distinct label $\bot$ to represent the \emph{lack of} an edge between two vertices; vertices $v$ and
$w$ are said to be \emph{adjacent} if $e(v, w) \ne \bot$ or $e(w, v) \ne \bot$. We write
...
neighbourhood is empty. A \emph{path} between two vertices $v$ and $w$ is a sequence of $n$ vertices
$v = v_1, v_2, \ldots, v_n = w$ such that for each $m \in \{ 1 \ldots n - 1 \}$, $v_m$ is adjacent
to $v_{m + 1}$. A graph is \emph{connected} if a path exists between each distinct pair of vertices.
...
equivalently, an independent set or a vertex cover ?? cite) in an association graph (which we describe
...
\cite{DBLP:journals/anor/PivaS12}, and heuristics ?? cite; SAT encodings seem to struggle even for
subgraph isomorphism??  cite IJCAI.)
...
\begin{figure}[tb]
    \centering
    \input{gen-graph-connected-cp-undir33.tex}
    \caption{A pairwise comparison of different CP techniques for the connected, undirected, 33\%
        labelled variant with graphs of up to 80 vertices. The colour at a point indicates the
        number of instances with these runtimes, with darker colours representing more instances.}
        \label{figure:connected-cp-undir33}
\end{figure}
...
\begin{figure}[tb]
    \centering
    \input{gen-graph-connected-cp-plain.tex}
    \caption{Different CP techniques, connected, undirected, unlabelled, up to 35 vertices.}
        \label{figure:connected-cp-plain}
\end{figure}
