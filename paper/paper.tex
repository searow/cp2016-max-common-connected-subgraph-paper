...
?? Connected, not strongly connected. No directed graphs.
...
\begin{figure}[tb]
    ...
    \caption{On top, the cumulative number of MCCS instances solved in under a certain time using
        different CP techniques. Below, instance-by-instance comparisons.}
...
filtering (Both). We show only results using the best variant for each class---that is, MAC for
labelled graphs, and FC for unlabelled graphs (the other results are very similar).  On labelled
graphs, we see many instances which are solved by branching but not filtering, and vice versa.
However, combining both is rarely much worse than just doing one or the other, and is often much
better, even if on average it is slightly slower.  On unlabelled graphs, we observe that the three
variants have rather similar performance.
...
rule without also adapting the combined bound / ordering heuristic.)
...
problems, particularly when edge labels are involved.

We believe there is further scope for tailoring clique algorithms for association graphs.
Specialised inference, association-aware bound function. Treating the first branch specially, since
it will eliminate so much of the search space \cite{DBLP:conf/cocoon/SutersAZSSL05}.

For CP models for the connected version of the problem, we saw the benefits of combining
conventional filtering with a special branching rule. Using a branching rule for correctness, rather
than simply as an ordering heuristic, is unconventional and does not cleanly fit into the
abstractions used by toolkits.
...
In particular, EHPs \cite{DBLP:journals/topc/McCreeshP15}. On the CP side, decomposition \cite{DBLP:conf/ictai/MinotNS15}.
...
\FloatBarrier % looks better this way...
