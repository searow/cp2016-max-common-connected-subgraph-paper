...
\vspace{1ex}
\nl $\FuncSty{expand}$ :: (Graph $G$, Set $\mathit{solution}$, Set $\mathit{adjacent}$, Set $\mathit{remaining}$) \;
\nl \Begin{
    \nl $\mathit{colourClasses} \gets \FuncSty{concatenate}($ \;
    $\hspace*{2em}\FuncSty{colour}(G, \mathit{remaining} \setminus \mathit{adjacent}),$ \;
    $\hspace*{2em}\FuncSty{colour}(G, \mathit{remaining} \cap \mathit{adjacent}))$ \;
