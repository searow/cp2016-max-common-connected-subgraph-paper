...
plot \
    ...
    "connected-undir33.data" u (sprintf("%.1e",$3*1000)+0):($3>=3600?1e-10:1e-3) smooth cumulative ti "MAC Branching" lc rgb '#AE017E' dt 3, \
    "connected-undir33.data" u (sprintf("%.1e",$2*1000)+0):($2>=3600?1e-10:1e-3) smooth cumulative ti "MAC Filtering" lc rgb '#DD3497' dt 4
...
plot \
    ...
    "connected-plain.data" u (sprintf("%.1e",$6*1000)+0):($6>=3600?1e-10:1e-3) smooth cumulative ti "FC Branching" lc rgb '#41AB5D' dt 6, \
    "connected-plain.data" u (sprintf("%.1e",$5*1000)+0):($5>=3600?1e-10:1e-3) smooth cumulative ti "FC Filtering" lc rgb '#78C679' dt 7
